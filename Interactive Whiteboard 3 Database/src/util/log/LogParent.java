...
public class LogParent
{
	...
	final protected static void load(String filename) throws IOException
	{
		...
		FileReader historyDataFileReader = new FileReader(filename);
		BufferedReader historyDataBufferedReader = new BufferedReader(historyDataFileReader);
		Scanner s = new Scanner(historyDataBufferedReader);
		//read every line
		while (s.hasNextLine())
		{
			m_logHistory.add(s.nextLine());
		}
		historyDataFileReader.close();
		historyDataBufferedReader.close();
		s.close();
	}
